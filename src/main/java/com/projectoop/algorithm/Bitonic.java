...
public class Bitonic extends Algorithm {
    ...
    public int bitonic() {
        ...
        step1.addStep(new VertexDetailStep(
                "Start from smallest (x, y) vertex = " + list.get(0).getIdVertex() + ".", true, list.get(0)
        ));
        int tempDist;
        ...
        step2.addStep(new VertexDetailStep(
                "Try Left = " + list.get(0).getIdVertex() + " to Right = " + list.get(1).getIdVertex() + ".", true, list.get(1)
        ));
        stackEdges.add(getData().getEdgeById(list.get(0).getIdVertex(), list.get(1).getIdVertex()));
                ...
                if (j <= (i + 1)) {
                    for (int k = 0; k < i; k++) {
                        ...
                        step3.addStep(new VertexDetailStep(
                                "Try Left = " + list.get(k).getIdVertex() + " to Right = " + list.get(j).getIdVertex() + ".", true, list.get(j)
                        ));
                        stackEdges.add(getData().getEdgeById(list.get(k).getIdVertex(), list.get(j).getIdVertex()));
                        step3.addStep(new EdgeDetailStep("", true, stackEdges.get(stackEdges.size() - 1)));
                        ...
                        if (min > temp) {
                            ...
                            step4.addStep(new EdgeDetailStep("", false, stackEdges.get(stackEdges.size() - 1)));
                    ...
                    pseudoSteps.add(step3);
                    step3.addStep(new VertexDetailStep(
                            "Try Right = " + list.get(j -1).getIdVertex() + " to Left = " + list.get(j).getIdVertex() + ".", true, list.get(j - 1)
                    ));
                    stackEdges.add(getData().getEdgeById(list.get(j-1).getIdVertex(), list.get(j).getIdVertex()));
                    step3.addStep(new EdgeDetailStep("", true, stackEdges.get(stackEdges.size() - 1)));
        ...
        step5.addStep(new VertexDetailStep(
                ...
                true, list.get(list.size() - 1)
        ));
        stackEdges.add(getData().getEdgeById(list.get(list.size() - 1).getIdVertex(), list.get(0).getIdVertex()));
        step5.addStep(new EdgeDetailStep("", true, stackEdges.get(stackEdges.size() - 1)));
